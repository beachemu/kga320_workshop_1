...
cat("success!") # Print "success!"
```

Using `str(barossa_data)`, we have created an output showing the structure of `paleo_data` in R.
...
Note the `na.rm = TRUE` argument. This tells R to ignore missing values (NAs) in our data. Now is a good time to start playing with variables. See if you can modify the code above to find the descriptive statistics for precipitation, `pr`.
